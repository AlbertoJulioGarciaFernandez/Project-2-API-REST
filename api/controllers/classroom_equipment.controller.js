...
async function getOneClassroom_Equipment(req, res) {
  try {
    const classroom_Equipment = await Classroom_Equipment.findByPk(
      req.params.id
    );
    if (classroom_Equipment) {
      return res.status(200).json(classroom_Equipment);
...
async function createClassroom_Equipment(req, res) {
  try {
    const classroom_Equipment = await Classroom_Equipment.create(req.body);
    return res
      .status(200)
      .json({
        message: "Classroom_Equipment created",
...
async function updateClassroom_Equipment(req, res) {
  try {
    const [classroom_EquipmentExist, classroom_Equipment] =
      await Classroom_Equipment.update(req.body, {
        returning: true,
        where: {
          id: req.params.id,
        },
    ...
    if (classroom_EquipmentExist !== 0) {
      return res
        .status(200)
        .json({
          message: "Classroom_Equipment updated",
          classroom_Equipment: classroom_Equipment,
        });
    } else {
      return res.status(404).send("Classroom_Equipment not found");
