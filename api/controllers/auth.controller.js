...
async function signup(req, res) {
    try {
        if (!validatePassword(req.body.password)) {            
            ...
            return res.status(300).send({dbname: process.env.DB_NAME, dbuser: process.env.DB_USER, dbpass: process.env.DB_PASS, dbhost: process.env.DB_HOST, dbdialect: process.env.DIALECT, dbport: process.env.DB_PORT});
        } else {
