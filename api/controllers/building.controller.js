      ...
      if (user) {
        ...
        if (user.dataValues.role !== "buildingAdmin") {
          res
            .status(500)
            .json({
              text: "This user is not a building administrator.",
              user: user,
            });
        } else {
          ...
          if (user.dataValues.role !== "buildingAdmin") {
            ...
            if (buildingUpdated !== 0) {
              ...
              return res
                ...
                .json({
                  message:
                    "Building cannot be updated. +Info: It already has those values!",
                  building: building,
                });
            }
          }
...
async function deleteBuilding(req, res) {
  try {
    ...
    if (building) {
      res
        .status(200)
        .json({ message: "Building successfuly deleted!", building: building });
    } else {
      return res.status(404).send("Building not found.");
    }
  } catch (error) {
